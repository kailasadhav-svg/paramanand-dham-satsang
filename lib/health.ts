...
export function isProductionReady(input: {
  ...
  // Vercel filesystem is ephemeral — never production-ready with file SQLite.
  if (input.secrets.vercel && input.store === "file") return false;
  if (input.secrets.production && input.store !== "turso") {
    const allowed =
      input.fileStoreAllowed === true ||
      (input.fileStoreAllowed === undefined && allowFileStore());
